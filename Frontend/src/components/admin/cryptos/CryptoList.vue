<template>
  <div>
    ...
    <!-- Bouton "Voir plus" si des cryptos sont présentes -->
    <button v-if="displayedCryptos.length < cryptos.length" @click="showMore" class=" text-blue-500 mt-0 underline ml-12">See More</button>
  </div>
</template>
  ...
  methods: {
    async fetchCryptos() {
      try {
        ...
        this.displayedCryptos = this.cryptos.slice(0, this.limit); // Initial display
      } catch (err) {
    ...
    showMore() {
      this.limit += 6; // Add 6 more cryptos each time
      this.displayedCryptos = this.cryptos.slice(0, this.limit);
    },
